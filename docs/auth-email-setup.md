...
- DNS-записи опубликованы и синхронизированы на `ns1.hosting.reg.ru` и `ns2.hosting.reg.ru`; домен Resend имеет статус `verified`, проверки DKIM и SPF пройдены.
...
Production-запрос восстановления вернул `200`, реальное письмо доставлено с русской темой и отправителем `Пора в Аргентину <no-reply@goargentina.ru>`. Финальная ручная проверка: новый пароль → новый вход → `/profile`.
