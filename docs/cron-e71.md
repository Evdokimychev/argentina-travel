...
| `POST/GET /api/cron/privacy/process` | `*/5 * * * *` (через maintenance-оркестратор) | Автоматическая обработка подтверждённых GDPR-запросов (E95) |
...
| `/api/cron/platform-maintenance` | `0 3 * * *` | typing cleanup; GDPR; digest; freshness; booking/trip-prep reminders; backup по воскресеньям |

Отдельные маршруты (`/api/cron/notifications/digest`, `/api/cron/messaging/cleanup-typing`, `/api/cron/privacy/process`, `/api/cron/ops/backup-hint`, `/api/cron/tripster-sync`, `/api/cron/sputnik8-sync`) остаются для **ручного** POST/GET с `CRON_SECRET`.
...
```json
{
  "crons": [
    ...
    { "path": "/api/cron/platform-maintenance", "schedule": "0 3 * * *" }
  ]
}
```
...
На Hobby-плане — до 2 cron-задач и **только расписание «раз в сутки»** (`0 H * * *`). Выражения вроде `*/5 * * * *` **блокируют деплой** на Hobby. Для частых задач (typing cleanup каждые 5 мин) используйте GitHub Actions или Pro-план. Ручной POST с `CRON_SECRET` доступен в любое время.
