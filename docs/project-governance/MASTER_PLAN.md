...
Обновлён: **2026-07-29 15:05 ART**. WP-022 exact candidate `4aa7f52c` ограничивает project-wide `exceed_egress_quota` amplification на public catalog read paths: один cold probe, quota-only open/half-open circuit, bounded logs и отсутствие 300-секундного cache poison. Local serial/parallel browser проходит 17/17, три cold details дают один circuit transition; Vercel build пока pending. Production data plane и schema parity остаются главными блокерами.
...
| 2F | Bound public catalog REST quota amplification (WP-022) | P1 | quota-only cold/open/half-open circuit; no result/token sharing; bounded logs/cache; exact local+preview recovery proof | implemented `4aa7f52c`; audit 2 113 + evidence 31, build 929/929, serial/parallel 17/17, one cold transition; Vercel pending, no deployment ID |
| 2G | Bound public CMS outage latency/log amplification (WP-023) | P1 | 25–28s CMS routes reproduced; fail-fast preserves unavailable/content truth; strict/recovery smoke within budget | next independent packet; root-cause trace pending |
| 2H | Dev-toolchain dependency remediation | P1 | full audit high=0 or explicit bounded exception with compatibility tests; production dependency graph remains clean | 9 high in `brace-expansion/minimatch/eslint`; only advertised full fix is breaking ESLint 10 `--force`, not applied |
...
- WP-022 exact local evidence closes single-process catalog quota amplification without sharing catalog/token data or masking schema/RLS errors. Parallel browser then isolated the next independent path: CMS/blog/destination/place fallbacks still take roughly 25–28 seconds and prevent recovery smoke completion, so WP-023 moves next while the external data plane remains blocked.
- Full dependency audit also exposed a dev-only ESLint transitive chain with 9 high findings. Production dependencies remain at 0; the available force fix is a breaking ESLint 10 upgrade, so dependency remediation follows WP-023 with compatibility evidence instead of being mixed into the reliability packet.
