...
## D-047 — A failed CMS read is unavailable, never evidence of absence

- Date: 2026-07-29
- Decision: every public CMS read distinguishes successful data (including confirmed `null`/`[]`) from operational unavailability. Known static editorial fallback may preserve the page, but it is applied outside the cache that stores successful CMS results. A CMS-only resolver must propagate typed unavailability so existence/detail routes return retryable 503 rather than false 404. Comments use the same truth rule and render a recoverable outage state instead of confirmed-empty copy.
- Evidence: before WP-024, single/list/detail resolvers and comments shared `null`/`[]` for query error and successful absence, while the blog catalog cache wrapped the fallback for 300 seconds. Exact `d4fbbbc1` passes **72 focused/related**, **2 133 full** and **31/31 evidence** tests, protected **929/929** build, local recovery smoke and local browser QA. Immutable `9nLoBaDbY1Aq6nnKkgPdPEXypT2e` binds the full SHA and returns article/comments/CMS-only **200/503/503** with the outage UI and no framework overlay.
- Consequence: P1-GA-021/R-033 are mitigated on the exact preview, not declared production-closed: old production still returns false-empty comments/catalog and the preview data plane is down. Healthy recovery and same-artifact promotion remain mandatory. Build evidence also registers P1-GA-022/WP-026 because a separate legacy Tripster logger still emits raw provider text; the new CMS logger itself is bounded and rate-limited.

## D-048 — Bound the unavoidable dev advisory; do not ship an incompatible override

- Date: 2026-07-29
- Decision: retain the current runtime/lock graph and accept only the exact dev-only ESLint advisory graph through 2026-08-12 under a blocking fail-closed repository policy. Production vulnerabilities, package/source/severity drift, a direct root crossing into production, expiry, or a clean upstream graph with a stale exception all fail the release gate. Remove the exception when supported upstream plugins no longer require the vulnerable minimatch/brace-expansion branch; do not extend it without fresh registry, audit and compatibility evidence.
- Evidence: production `npm audit` is 0; full audit is exactly nine high packages sourced only from advisory `1124334`. `brace-expansion@5.0.8` exports `{ expand }` while `minimatch@3` invokes the CommonJS module as a function, so a global override breaks the contract. Current latest React/import/jsx-a11y ESLint plugins still depend on minimatch 3; Next 16/ESLint 10 does not close those branches and separately removes `next lint`, changes build defaults and requires migration. Exact `5c79c4cb` passes 5 policy tests, real audits, 2 133 unit tests, 31 quick evidence contracts, 50 static-gate contracts and a 929/929 protected build.
- Consequence: P1-GA-019/R-032 move from unbounded open finding to accepted time-bounded exception, not resolved. CI now exposes any drift and forces review by 2026-08-12. Vercel rejected the exact SHA with `Account is blocked`, so no preview/deployment evidence is claimed; production remains unaffected and clean.

## D-049 — Provider text is internal classification data, never release telemetry
