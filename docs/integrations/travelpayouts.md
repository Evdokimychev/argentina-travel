...
Клиент ограничивает ожидание Links API десятью секундами и принимает ответ только целиком:
число ссылок, `marker` и `trs` должны совпасть с запросом, а каждая ссылка должна иметь
`code: success` и корректный HTTP(S)-адрес. Частичный или неоднозначный ответ считается
ошибкой и не используется для перенаправления.
