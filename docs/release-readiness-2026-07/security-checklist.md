...
**Итог повторной реализации:** **BLOCKED НА МИГРАЦИЯХ И STAGING-ПРОВЕРКЕ.** Первоначальные SEC-001–SEC-006 ниже сохранены как baseline-находки. Они закрыты кодом/миграцией `20260715032401_secure_auth_role_bootstrap`: регистрация не доверяет metadata role, lookup нейтрализован, OTP атомарен, completed privacy metadata не сохраняет исходные email/name, attribution зависит от consent. Дополнительно закрыты cron header spoofing, self organizer upgrade, conversation participant writes, публичная отправка поддельных писем и JSON-LD injection.

Новые service-role-only таблицы partner idempotency и email outbox, а также role/RLS migrations ещё не применены production. `npm audit --omit=dev`: 0 critical/high, 2 moderate в вложенном PostCSS Next.js. `supabase:verify` завершился `fetch failed`; считать live security подтверждённой нельзя.
