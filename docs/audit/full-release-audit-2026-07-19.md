...
- Исторический разрыв migration history закрыт доказанным baseline `production-2026-07-19-v1`. Manifest фиксирует project ref, PostgreSQL 17.6, checksum первых 102 миграций и SHA-256 fingerprint 3 634 объектов public-схемы. В закрытой `app_migrations` одной транзакцией записаны 102 checksum-строки и baseline metadata; повторный apply идемпотентен, `anon/authenticated` не имеют schema usage. Будущие миграции применяются только проектным fail-closed runner, прямой `supabase db push` запрещён.
...
1. В production-окружении Vercel отсутствует `CRON_SECRET`. Защищённые cron-маршруты корректно fail-closed отвечают `401`, поэтому автоматические синхронизации и публикации не заработают, пока владелец не добавит отдельный случайный секрет не короче 32 символов и не выполнит новый deploy. Значения секретов во время аудита не читались и не менялись.
2. Требуется отозвать/проверить старые ключи, найденные только в закрытом восстановленном архиве.
3. Restore rehearsal полной копии ещё требует отдельного disposable Supabase target. Production backup создан и каталог dump проверен, но наличие backup не заменяет фактическое восстановление.
4. Native Tripster External Orders и YouTravel booking API недоступны по текущим партнёрским правам; внешний checkout работает.
5. GTM/GA4/Метрика/Search Console требуют действий владельца в Vercel и кабинетах провайдеров.
...
Код, контент, публичные маршруты, база, RLS, checksum-backed migration baseline, бронирование и партнёрские fallback-сценарии готовы. Внешние аналитические и партнёрские ограничения не скрыты и не требуют блокировать текущий контентный/UX-релиз. Автоматические cron-процессы при этом остаются отдельным красным операционным пунктом до добавления владельцем `CRON_SECRET`.
