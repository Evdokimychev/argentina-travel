---
id: skam-s-falshivoj-policiej
type: faq
question: "Что делать, если в Аргентине «полицейский» просит показать деньги или пройти к банкомату?"
short_answer: "Не передавайте наличные, карту, PIN-код или телефон для «проверки» и не идите к банкомату. Не сопротивляйтесь физически: держите руки на виду, спокойно попросите удостоверение и объяснение, перейдите в людное место и, если это безопасно, самостоятельно позвоните 911 для проверки и помощи."
topic: bezopasnost
status: published
site_ready: false
editorial_hold_reason: "I7 карантин: просрочена claim-level проверка (stale_sensitive_claim / stale_source_url_check / expired_source). Даты не обновлялись без фактчека; нужна повторная сверка источников."
site_sections: [puteshestviya-po-argentine, zhizn-v-strane]
tags: [безопасность, мошенничество, полиция, наличные]
related: [bezopasnost-argentina]
last_verified: "2026-07-20"
confidence: medium
provenance:
  schema_version: 1
  mode: strict
  stale_after_days: 30
sources:
  - id: argentina-911-fake-police
    title: "Ministerio de Seguridad Nacional — 911 Emergencias"
    url: "https://www.argentina.gob.ar/seguridad/911emergencias"
    lang: es
    type: official
    authority: primary
    url_status: verified
    checked_at: "2026-07-20"
    expires_at: "2026-08-20"
  - id: bcra-authorized-exchange-fake-police
    title: "BCRA — валютные операции через уполномоченные организации"
    url: "https://www.bcra.gob.ar/noticias/el-bcra-aclara-que-no-hay-ningun-cambio-normativo-para-la-compra-de-dolares-por-parte-de-las-personas-humanas/"
    lang: es
    type: official
    authority: primary
    url_status: verified
    checked_at: "2026-07-20"
    expires_at: "2026-08-20"
  - id: canada-argentina-cash-safety
    title: "Government of Canada — безопасность у банкоматов и обменных пунктов в Аргентине"
    url: "https://travel.gc.ca/destinations/argentina"
    lang: en
    type: official
    authority: primary
    url_status: verified
    checked_at: "2026-07-20"
    expires_at: "2026-08-20"
claims:
  - id: call-911-to-verify-emergency
    text: "Для сообщения об экстренной ситуации и запроса полиции в Аргентине используется номер 911."
    sensitive: true
    source_ids: [argentina-911-fake-police]
    verified_at: "2026-07-20"
    reviewer: { id: "goargentina-editorial", role: "Редакционная проверка источников" }
  - id: authorized-currency-exchange-only
    text: "Официальные валютные операции проводят банки и уполномоченные обменные организации, а не человек на улице, предлагающий проверить или обменять купюры."
    sensitive: true
    source_ids: [bcra-authorized-exchange-fake-police]
    verified_at: "2026-07-20"
    reviewer: { id: "goargentina-editorial", role: "Редакционная проверка источников" }
  - id: atm-and-exchange-following-risk
    text: "Канадская государственная рекомендация предупреждает, что преступники могут ждать у банкоматов и обменных пунктов или следовать за человеком после получения наличных."
    sensitive: true
    source_ids: [canada-argentina-cash-safety]
    verified_at: "2026-07-20"
    reviewer: { id: "goargentina-editorial", role: "Редакционная проверка источников" }
---

## Ответ

Сценарий, в котором незнакомый человек показывает удостоверение и требует наличные «для проверки подлинности», карту, PIN-код или поездку к банкомату, следует считать опасным до независимой проверки. В старом тексте эта схема без источника была названа «регулярной в туристических зонах» и утверждалось, что полиция «никогда» не просит деньги. Надёжной аргентинской статистики для таких абсолютных формулировок редакция не нашла, поэтому мы их убрали.

Действуйте так:

1. Не доставайте кошелёк и не передавайте телефон, карту, наличные или код.
2. Не делайте резких движений и не вступайте в физический спор. Держите руки на виду.
3. Спокойно попросите назвать подразделение, причину обращения и показать удостоверение так, чтобы не отдавать свои вещи.
4. Если можно безопасно отойти, перейдите к сотрудникам открытого отеля, магазина, банка или официальной стойки.
5. Самостоятельно наберите **911**, опишите место и человека и попросите подтвердить ситуацию или направить помощь. Не звоните по номеру, который диктует собеседник.

Если вам угрожают оружием или силой, приоритет — жизнь и здоровье, а не сохранение имущества или доказательство своей правоты. Не сопротивляйтесь ограблению; запомните место и приметы настолько, насколько это возможно без дополнительного риска.

Обмен валюты проводите только в банке или уполномоченном обменном доме. После банкомата или получения крупной суммы не пересчитывайте деньги на улице и проверьте, не идёт ли кто-то следом. При подозрении зайдите в охраняемое помещение. После инцидента заблокируйте карту и SIM, сохраните время и место, подайте заявление и получите его номер для банка, страховщика или консульства.

## Источники

- [Ministerio de Seguridad Nacional — 911](https://www.argentina.gob.ar/seguridad/911emergencias).
- [BCRA — уполномоченные каналы обмена валюты](https://www.bcra.gob.ar/noticias/el-bcra-aclara-que-no-hay-ningun-cambio-normativo-para-la-compra-de-dolares-por-parte-de-las-personas-humanas/).
- [Government of Canada — безопасность в Аргентине](https://travel.gc.ca/destinations/argentina).
