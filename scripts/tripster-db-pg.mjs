...
export async function pgInsertSyncRun(client) {
  const { rows } = await client.query(
    `insert into public.tripster_sync_runs (status) values ('running') returning id`
  );
  return rows[0].id;
}
...
export async function pgFinishSyncRun(client, syncRunId, payload) {
  await client.query(
    `update public.tripster_sync_runs
     set status = $2,
         finished_at = $3,
         cities_synced = $4,
...
export async function pgUpsertCountry(client, country) {
  await client.query(
    `insert into public.tripster_countries (id, slug, name_ru, name_en, currency, experience_count, payload)
    ...
    [
      country.id,
      country.slug ?? null,
      country.name_ru ?? null,
      country.name_en ?? null,
      country.currency ?? null,
      country.experience_count ?? 0,
...
export async function pgUpsertCities(client, cityRows) {
  for (const city of cityRows) {
    await client.query(
      `insert into public.tripster_cities (id, country_id, slug, name_ru, name_en, experience_count, cover_image, payload)
...
export async function pgFetchExistingExperienceIds(client) {
  const { rows } = await client.query(`select id, slug from public.tripster_experiences`);
  return new Map(rows.map((row) => [row.id, row.slug]));
}

export async function pgUpsertExperiences(client, rows) {
  for (const row of rows) {
    await client.query(
      `insert into public.tripster_experiences (
         ...
         price_currency, price_display, tripster_url, partner_url, cover_image, photos, payload, synced_at
       ) values (
       ...
       on conflict (id) do update set
         ...
         tripster_url = excluded.tripster_url,
