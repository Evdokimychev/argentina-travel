...
describe("content governance publication boundary", () => {
  ...
  it("persists a scheduled draft before checking the database gate", () => {
    const scheduleStart = contentServer.indexOf("export async function scheduleCmsDocument");
    const scheduleEnd = contentServer.indexOf("export async function cancelScheduledCmsDocument");
    const scheduleSource = contentServer.slice(scheduleStart, scheduleEnd);
    expect(scheduleSource.indexOf("await updateCmsDocument")).toBeGreaterThan(0);
    expect(scheduleSource.indexOf("await updateCmsDocument")).toBeLessThan(
      scheduleSource.indexOf("checkCmsPublicationGate")
    );
  });
