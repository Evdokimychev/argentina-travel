...
export function buildCatalogMetadata(
  ...
  const description =
    count === 0
      ? filterParts.length > 0
        ? `Сейчас нет туров по выбранным фильтрам (${filterParts.join(", ")}). Посмотрите полный каталог путешествий по Аргентине.`
        : "Авторские туры и экскурсии по Аргентине — от Буэнос-Айреса до Патагонии. Фильтры по датам, цене, формату и региону."
      : filterParts.length > 0
