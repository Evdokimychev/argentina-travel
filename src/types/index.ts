...
export interface BlogPost {
  ...
  displayOptions?: {
    ...
    /**
     * Рендерит каждый BlogPostSection как отдельную панель (фон, рамка, отступы)
     * вместо простого разделения пробелом. Помогает длинным гидам не превращаться
     * в сплошное белое полотно текста. По умолчанию выключено (не меняет существующие статьи).
     */
    sectionPanels?: boolean;
  };
}
