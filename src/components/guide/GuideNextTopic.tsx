...
export default function GuideNextTopic({ slug }: GuideNextTopicProps) {
  const index = GUIDE_TOPIC_ORDER.indexOf(slug);
  if (index === -1) return null;

  const prevSlug = index > 0 ? GUIDE_TOPIC_ORDER[index - 1] : null;
  const nextSlug = index < GUIDE_TOPIC_ORDER.length - 1 ? GUIDE_TOPIC_ORDER[index + 1] : null;

  if (!prevSlug && !nextSlug) return null;

  const prevTopic = prevSlug ? GUIDE_TOPICS[prevSlug] : null;
  const nextTopic = nextSlug ? GUIDE_TOPICS[nextSlug] : null;

  return (
    <nav className="grid gap-3 sm:grid-cols-2" aria-label="Соседние темы путеводителя">
