...
import { BlogRichGallerySection } from "@/components/blog/BlogRichArticleClientBlocks";
...
import type { BlogPost, BlogPostSection, TourListing } from "@/types";
...
import { adaptRichArticleToBlogSections } from "@/editorial/adapters/blog-body";
...
export default function BlogPostView({
  ...
  const richArticle = post.richArticleId
    ...
    : undefined;
  const richBodySections = richArticle
    ? adaptRichArticleToBlogSections(richArticle)
    : undefined;
  const effectiveSections: Array<BlogPostSection & { anchorId?: string }> =
    richBodySections ??
  ...
  const sectionsWithIds = effectiveSections.map((section) => {
    const headingId =
      section.anchorId && !usedIds.has(section.anchorId)
        ? section.anchorId
        : headingToAnchorId(section.title, usedIds);
  ...
  const tocItems = sectionsWithIds.map(({ section, headingId }) => ({
    id: headingId,
    label: stripHeadingDecorations(section.title),
    level: 2 as const,
  }));
  const sectionCount = sectionsWithIds.length;
  ...
  const inlineRelatedBySection =
    !post.noIndex && sectionsWithIds.length
      ? buildInlineRelatedPostsBySection(
          post,
          catalog,
          sectionsWithIds.map(({ section }) => section),
          ({ section }, excludeSlugs) =>
  ...
  const richGalleryImages =
    richArticle && post.richArticleId
              ...
              {richArticle ? (
                <div className="space-y-10">
                  ...
                  <div className="rounded-2xl border border-sky/15 bg-gradient-to-br from-sky/[0.07] via-white to-white p-5 shadow-sm sm:p-6">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        {richArticle.intro?.map((paragraph, index) => (
                          <p
                            key={`${index}-${paragraph}`}
                  ...
                  {sectionsWithIds.map(({ section, headingId }, index) => (
                    <BlogPostSectionView
                      key={section.anchorId ?? section.title}
                      section={section}
                      headingId={headingId}
                      index={index}
                      post={post}
