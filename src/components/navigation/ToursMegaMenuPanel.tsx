...
function ToursPromoBlock({ onNavigate }: { onNavigate?: () => void }) {
  return (
    <aside className="flex flex-col rounded-xl bg-gradient-to-br from-sky/10 via-surface-muted/80 to-surface-muted/40 p-4 sm:p-5 lg:min-h-full">
      ...
      <p className="mt-2 text-sm leading-relaxed text-slate">
        Групповые и индивидуальные маршруты с русскоязычными организаторами.
      </p>
      <Link
        href="/tours"
        onClick={onNavigate}
...
function tourIcon(_link: SiteNavLink) {
  return Compass;
}

export function ToursMegaMenuPanel({
  columns,
  t,
  onNavigate,
  className,
  ...
  return (
    <div className={cn("flex flex-col", className)}>
      ...
      >
        <ToursPromoBlock onNavigate={onNavigate} />
        <MegaMenuPanel
          columns={columns}
          t={t}
          onNavigate={onNavigate}
          showIcons
          resolveIcon={tourIcon}
