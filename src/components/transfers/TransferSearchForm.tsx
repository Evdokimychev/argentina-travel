...
import TransferLocationCombobox from "@/components/transfers/TransferLocationCombobox";
import { buttonVariants } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import type { TransferLocation } from "@/lib/intui/types";
import type { LocaleCode } from "@/types/locale";

export type TransferSearchFormState = {
...
type TransferSearchFormProps = {
  state: TransferSearchFormState;
  onChange: (next: TransferSearchFormState) => void;
  onSubmit: () => void;
  loading?: boolean;
  locale: LocaleCode;
  labels: {
    ...
    >
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <label className="block">
          <span className="mb-1.5 block text-xs font-medium text-charcoal">{labels.date}</span>
          <input
            type="date"
            min={minDate}
            value={state.date}
        ...
        <label className="block">
          <span className="mb-1.5 block text-xs font-medium text-charcoal">{labels.time}</span>
          <input
            type="time"
            value={state.time}
      ...
      <button
        type="submit"
        disabled={loading || !state.origin || !state.destination || !state.date}
        className={cn(
