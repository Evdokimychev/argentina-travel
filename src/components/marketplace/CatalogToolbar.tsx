...
import { filtersWord } from "@/lib/pluralize";
...
export type CatalogSortOption<V extends string = string> = {
  value: V;
  label: string;
};

interface CatalogToolbarProps<V extends string = TourSortOption> {
  countLabel: string;
  sort: V;
  onSortChange: (sort: V) => void;
  primarySortOptions?: CatalogSortOption<V>[];
  secondarySortOptions?: CatalogSortOption<V>[];
...
export default function CatalogToolbar<V extends string = TourSortOption>({
  countLabel,
  sort,
  onSortChange,
  primarySortOptions = PRIMARY_SORT_OPTIONS as CatalogSortOption<V>[],
  secondarySortOptions = SECONDARY_SORT_OPTIONS as CatalogSortOption<V>[],
  viewMode = "grid",
  onViewModeChange,
  showViewToggle = true,
  activeFilterCount = 0,
  ...
  const secondaryLabel =
    secondarySortOptions.find((option) => option.value === sort)?.label ?? "Ещё";
