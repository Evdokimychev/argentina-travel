...
import { DIFFICULTY_DOT_COUNT, DIFFICULTY_ICONS, DIFFICULTY_LEVELS } from "@/data/tour-levels";
import { DifficultyDotRating } from "./sidebar-filter-ui";
...
interface DifficultyFilterProps {
  ...
  counts: Partial<Record<DifficultyLevel, number>>;
  ...
  return (
    <>
      <div className="border-b border-gray-100 px-4 py-3">
        <p className="text-sm font-semibold text-charcoal">Физическая нагрузка</p>
        <p className="mt-0.5 text-xs text-slate">лёгкая → экстремальная</p>
      </div>
            ...
            <li key={level}>
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-[11px] leading-relaxed text-slate/90">{example}</p>
                  <DifficultyDotRating filled={dots} className="mt-1.5" />
                </span>
              </button>
            </li>
          );
