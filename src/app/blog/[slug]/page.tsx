...
export async function generateStaticParams() {
  try {
    ...
    return catalog.filter((post) => !isAutoGeneratedBlogPost(post)).map((post) => ({ slug: post.slug }));
  } catch {
    const cutover = await getCmsCutoverFlags();
    return cutover.blog
      ? []
      : blogPosts.filter((post) => !isAutoGeneratedBlogPost(post)).map((post) => ({ slug: post.slug }));
  }
}
...
export async function generateMetadata({ params }: BlogPostPageProps) {
  ...
  if (isAutoGeneratedBlogPost(post)) {
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  const { slug } = await params;
  const locale = await getServerI18nLocale();
  const [catalog, post] = await Promise.all([
  ...
  if (isAutoGeneratedBlogPost(post)) {
    if (post.canonicalSlug && post.canonicalSlug !== post.slug) {
      permanentRedirect(blogPostPath(post.canonicalSlug));
    }
    notFound();
  }
  ...
  return (
    <>
      ...
      <BlogPostView post={post} catalog={catalog} initialTours={initialTours} />
    </>
  );
}
