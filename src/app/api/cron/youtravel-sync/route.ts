...
const CRON_ROUTE = "/api/cron/youtravel-sync";
...
export async function GET(request: Request) {
  ...
  const scriptPath = path.join(root, "scripts/youtravel-sync.mjs");
  ...
  try {
    const { stdout, stderr } = await execFileAsync(process.execPath, [scriptPath], {
      ...
      env: {
        ...process.env,
        YOUTRAVEL_SKIP_AFFILIATE_LINKS: process.env.YOUTRAVEL_SKIP_AFFILIATE_LINKS ?? "true",
      },
    ...
    await logCronResult(CRON_ROUTE, {
      ok: true,
      ranAt,
      message: "YouTravel sync completed",
      statusCode: 200,
