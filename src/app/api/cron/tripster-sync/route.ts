...
const CRON_ROUTE = "/api/cron/tripster-sync";
...
export async function GET(request: Request) {
  ...
  const scriptPath = path.join(root, "scripts/tripster-sync.mjs");
  ...
  try {
    ...
    await logCronResult(CRON_ROUTE, {
      ok: true,
      ranAt,
      message: "Tripster sync completed",
      statusCode: 200,
