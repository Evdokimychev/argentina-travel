...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (health.ok) {
    try {
      ...
      preview = result.items.map((item) => ({ title: item.title ?? "Без заголовка", excerpt: (item.rawContent ?? "").slice(0, 300), sourceUrl: item.canonicalUrl ?? item.sourceUrl ?? null, publishedAt: item.publishedAt ?? null, mediaCount: (item.media?.length ?? 0) + (item.attachments?.length ?? 0) }));
    } catch {
      return NextResponse.json({ health: { ok: false, message: "Соединение установлено, но предварительный просмотр не получен" }, preview }, { status: 424 });
    }
  }
